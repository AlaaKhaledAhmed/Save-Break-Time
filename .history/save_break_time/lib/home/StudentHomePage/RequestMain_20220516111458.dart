...
//confirm order----------------------------------------------------
            ...
            buttoms(context, "Confirm orders", 14.0, black, () async {
              ...
              cardCollection
                ...
                for (int i = 0; i < cafeItem.length; i++) {
                  productCollection
                      .where("productID", isEqualTo: studentCafePrId[i])
                      .get()
                      .then((snapshot) {
                    for (DocumentSnapshot ds in snapshot.docs) {
                      if (ds["prQuantity"] < studentCafeQuantity[i]) {
                        //  showMessage(context, "سلة التسوق", " ${ds["prName"]} الكمية لاتكفي من المنتج");
                      } else {
                        ds.reference.update({
                          "prQuantity": ds["prQuantity"] - studentCafeQuantity[i],
                        });
                      }
                    }
                  });
