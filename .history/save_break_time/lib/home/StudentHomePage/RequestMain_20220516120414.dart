...
class _RequestMainState extends State<RequestMain> {
  ...
  List<int> studentCafePrId = [];
  List<int> studentCafeQuantity = [];
  List<int> studentBookPrId = [];
  List<int> studentBookQuantity = [];
  String currentUser;
  String userPhone;
  String name;
            ...
            buttoms(context, "Confirm orders", 14.0, black, () async {
              ...
              if (cafeItem.length > 0) {
                FirebaseFirestore.instance.collection("order").add({
                  ...
                  //update cafeteria quantity
                  for (int i = 0; i < studentCafePrId.length; i++) {
                    productCollection
                      ...
                      for (DocumentSnapshot ds in snapshot.docs) {
                        if (ds["prQuantity"] < studentCafeQuantity[i]) {
                          ...
                          ds.reference.update({
                            ...
                            //update book store quantity
                            for (int i = 0; i < studentBookPrId.length; i++) {
                              productCollection
                                  .where("prID", isEqualTo: studentBookPrId[i])
                                  .get()
                                  .then((snapshot) {
                                for (DocumentSnapshot ds in snapshot.docs) {
                                  if (ds["prQuantity"] <
                                      studentBookQuantity[i]) {
                                    dialog(context, "request",
                                        "Quantity is not enough");
                                  } else {
                                    ds.reference.update({
                                      "prQuantity": ds["prQuantity"] -
                                          studentBookQuantity[i],
                                    });
                                  }
                                }
...
//-----------------------------------------------------------
  refrash() {
    cafeItem.clear();
    bookItem.clear();
    studentCafePrId.clear();
    studentCafeQuantity.clear();
    studentBookPrId.clear();
    studentBookQuantity.clear();
    cardCollection
        .where('StudentId', isEqualTo: currentUser)
        .get()
        .then((value) {
      for (var element in value.docs) {
        if (element["type"] == "cafie") {
          ...
          studentCafePrId.add(element["prId"]);
          studentCafeQuantity.add(element["StudentQuantity"]);
        } else {
          bookItem.add(element.data());
          studentBookPrId.add(element["prId"]);
          studentBookQuantity.add(element["StudentQuantity"]);
        }
      }
    });
  }
