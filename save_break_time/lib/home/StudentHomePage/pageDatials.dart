                          ...
                          if (total <= 0) {
                            ...
                            if (x == 1) {
                              cardCollection.doc(docId).update({
                                "total price":
                                    dbtotalPrice + widget.price * total,
                                "StudentQuantity": dbQuantity + total,
                              }).then((value) {
...
//--------------------------------------------------
  void searchForProduct() {
    cardCollection.where('prId', isEqualTo: widget.prId).get().then((value) {
      setState(() {
        x = value.docs.length;
        for (var element in value.docs) {
          setState(() {
            docId = element.id;
            dbQuantity = element["StudentQuantity"];
